...
async function writeAppEvent(event, payload = {}) {
  try {
    await maintainerLogger?.write?.(event, payload);
  } catch {
    // Diagnostic logging must not block user-triggered recovery actions.
  }
}
...
function registerIpc() {
  ...
  ipcMain.handle("vpn:snapshot", async (_event, payload = {}) => {
    ...
    if (shouldAudit) {
      await writeAppEvent("manual-refresh-status-started", {
    ...
    if (shouldAudit) {
      await writeAppEvent("manual-refresh-status-completed", {
        ...
        gatewayCandidates: snapshot.environmentInfo?.gatewayCandidates ?? [],
      });
    }
    return snapshot;
  });
