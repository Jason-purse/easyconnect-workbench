...
function parseAppleScriptBoolean(value = "") {
  return `${value}`.trim().toLowerCase() === "true";
}

function parseNativeAlertSummaries(value = "") {
  return `${value}`
    .split("##ALERT##")
    .map((entry) => entry.trim())
    .filter(Boolean)
    .map((entry) => {
      const [text = "", buttonsRaw = ""] = entry.split("##BUTTONS##");
      return {
        text: text.trim(),
        buttons: buttonsRaw.split("||").map((button) => button.trim()).filter(Boolean),
      };
    })
    .filter((alert) => alert.text);
}

function isKnownBlockingNativeAlertText(text = "") {
  return (
    /ecResize/i.test(text) ||
    /onResizeWindow failed/i.test(text) ||
    /winID is not exist/i.test(text) ||
    /EasyConnect遇到严重错误/i.test(text) ||
    /严重错误/i.test(text)
  );
}

function nativeAlertMatchesDismissalRequest(alert = {}, expectedTexts = []) {
  ...
  return (
    isKnownBlockingNativeAlertText(text) ||
    expectedTexts.some((expected) => {
      const expectedText = `${expected ?? ""}`.trim();
      return expectedText && (text.includes(expectedText) || expectedText.includes(text));
    })
  );
}

function parseNativeAlertDismissalOutput(stdout = "") {
  const dismissed = Number.parseInt(`${stdout}`.trim(), 10);
  return Number.isFinite(dismissed) ? dismissed : 0;
}

function parseNativeWindowConsolidationOutput(stdout = "") {
  const normalized = `${stdout}`.trimEnd();
  const [runningRaw = "false", beforeRaw = "0", afterRaw = "0", closedRaw = "0"] = normalized.includes("##")
    ? normalized.split("##")
    : normalized.split("\n");
  const beforeCount = Number.parseInt(beforeRaw, 10);
  const afterCount = Number.parseInt(afterRaw, 10);
  const closed = Number.parseInt(closedRaw, 10);

  return {
    running: parseAppleScriptBoolean(runningRaw),
    beforeCount: Number.isFinite(beforeCount) ? beforeCount : null,
    afterCount: Number.isFinite(afterCount) ? afterCount : null,
  ...
  async getOfficialNativeWindowState() {
    if (process.platform !== "darwin") {
      return {
        ok: false,
        supported: false,
        reason: "Native EasyConnect window inspection is only supported on macOS.",
      };
    }

    const processName = path.basename(this.appExecutable);
    const script = `
tell application "System Events"
  ...
  tell application process "${processName}"
    set appVisible to visible as text
    set appWindowCount to count of windows
    set appWindowNames to ""
    set appAlerts to {}
    if appWindowCount > 0 then
      set AppleScript's text item delimiters to "||"
      set appWindowNames to name of windows as text
      set AppleScript's text item delimiters to ""
      repeat with appWindow in windows
        repeat with appSheet in sheets of appWindow
          ...
          try
            set AppleScript's text item delimiters to " "
            set alertText to value of static texts of appSheet as text
            set AppleScript's text item delimiters to ""
          end try
          try
            set AppleScript's text item delimiters to "||"
            set alertButtons to name of buttons of appSheet as text
            set AppleScript's text item delimiters to ""
          end try
          if alertText is not "" then
            set end of appAlerts to alertText & "##BUTTONS##" & alertButtons
          end if
        end repeat
      end repeat
    end if
    set AppleScript's text item delimiters to "##ALERT##"
    set appAlertsText to appAlerts as text
    set AppleScript's text item delimiters to ""
    return appVisible & "\n" & appWindowCount & "\n" & appWindowNames & "\ntrue\n" & appAlertsText
  end tell
end tell`;
    ...
    try {
      ...
      const [visibleRaw = "false", countRaw = "0", namesRaw = "", runningRaw = "false", alertsRaw = ""] = stdout.trimEnd().split("\n");
      const windowCount = Number.parseInt(countRaw, 10);

      return {
        ok: true,
        supported: true,
        running: parseAppleScriptBoolean(runningRaw),
        visible: parseAppleScriptBoolean(visibleRaw),
        windowCount: Number.isFinite(windowCount) ? windowCount : 0,
        windowNames: namesRaw ? namesRaw.split("||").filter(Boolean) : [],
        alerts: parseNativeAlertSummaries(alertsRaw),
      };
    } catch (error) {
      return {
        ok: false,
        supported: true,
        error: sanitizeCommandOutput(error?.stderr || error?.message || String(error)),
      };
    }
  }
  ...
  async dismissOfficialNativeAlerts(alerts = [], options = {}) {
    if (process.platform !== "darwin") {
      return {
        ok: false,
        supported: false,
        reason: "Native EasyConnect alert dismissal is only supported on macOS.",
      };
    }

    const expectedTexts = alerts
    ...
    if (expectedTexts.length === 0) {
      return {
        ...
        supported: true,
        dismissed: 0,
        reason: "No native alerts were supplied.",
      };
    }

    const processName = path.basename(this.appExecutable);
    const buildDismissScript = () => `
try
  ...
  tell application process "${processName}"
    ...
    repeat with appWindow in windows
      repeat with appSheet in sheets of appWindow
        ...
        if alertText contains "ecResize" or alertText contains "onResizeWindow failed" or alertText contains "winID is not exist" then
          try
            perform action "AXPress" of button "OK" of appSheet
            set dismissedCount to dismissedCount + 1
          on error
            try
              click button "OK" of appSheet
              set dismissedCount to dismissedCount + 1
            on error
              try
                perform action "AXPress" of button 1 of appSheet
                set dismissedCount to dismissedCount + 1
              on error
                try
                  click button 1 of appSheet
                  set dismissedCount to dismissedCount + 1
                end try
              end try
            end try
    ...
    return dismissedCount as text
  end tell
end tell`;

    try {
      const timeoutMs = options.timeoutMs ?? 5000;
      const maxDismissRounds = Math.max(1, Number.parseInt(`${options.maxDismissRounds ?? 6}`, 10) || 6);
      let dismissed = 0;
      let dismissRounds = 0;
      const parsedVerifyTimeoutMs = Number.parseInt(`${options.verifyTimeoutMs ?? 2500}`, 10);
      const parsedVerifyPollMs = Number.parseInt(`${options.verifyPollMs ?? 250}`, 10);
      const verifyTimeoutMs = Number.isFinite(parsedVerifyTimeoutMs) && parsedVerifyTimeoutMs >= 0
        ? parsedVerifyTimeoutMs
        : 2500;
      const verifyPollMs = Number.isFinite(parsedVerifyPollMs) && parsedVerifyPollMs > 0
        ? parsedVerifyPollMs
        : 250;
      ...
      do {
        dismissRounds += 1;
        const { stdout } = await execFileAsync("/usr/bin/osascript", ["-e", buildDismissScript()], {
          timeout: timeoutMs,
          maxBuffer: 32 * 1024,
        });
        dismissed += parseNativeAlertDismissalOutput(stdout);

        finalState = await this.getOfficialNativeWindowState();
        remainingAlerts = (finalState?.alerts ?? []).filter((alert) =>
          nativeAlertMatchesDismissalRequest(alert, expectedTexts),
        );
        if (finalState?.ok && remainingAlerts.length === 0) {
          return {
            ok: true,
            supported: true,
            dismissed: Number.isFinite(dismissed) ? dismissed : 0,
            requested: expectedTexts.length,
            verified: true,
            finalState,
          };
        }

        if (dismissRounds >= maxDismissRounds) {
          break;
        }

        if (finalState?.ok === false) {
          break;
        }
        ...
        await sleep(Math.max(50, verifyPollMs), options.signal);
      } while (true);
      ...
      return {
        ...
        requested: expectedTexts.length,
        verified: false,
        error: "Native EasyConnect alert is still visible after dismissal.",
        remainingAlerts,
        finalState,
      };
    } catch (error) {
  ...
  async closeExtraOfficialNativeWindows(options = {}) {
    if (process.platform !== "darwin") {
      return {
        ok: false,
        supported: false,
        reason: "Native EasyConnect window consolidation is only supported on macOS.",
      };
    }
...
tell application "System Events"
  ...
  tell application process "${processName}"
    ...
    repeat while (count of windows) > 1
      set appWindow to item 2 of windows
      try
        perform action "AXPress" of (first button of appWindow whose subrole is "AXCloseButton")
        set closedCount to closedCount + 1
      on error
        try
          click (first button of appWindow whose subrole is "AXCloseButton")
          set closedCount to closedCount + 1
        on error
          exit repeat
        end try
    ...
    return "true##" & beforeCount & "##" & afterCount & "##" & closedCount
  end tell
end tell`;
    ...
    try {
      ...
      const parsed = parseNativeWindowConsolidationOutput(stdout);
      const finalState = await this.getOfficialNativeWindowState();
      const finalWindowCount = finalState?.ok ? finalState.windowCount : parsed.afterCount;
      const ok = parsed.running && finalState?.ok && Number.isFinite(finalWindowCount) && finalWindowCount <= 1;
      ...
      return {
        ...
        finalState,
        ...(ok ? {} : { error: "Native EasyConnect windows are still not consolidated." }),
      };
    } catch (error) {
      return {
        ok: false,
  ...
  async showOfficialWindowPageTarget(target, options = {}) {
    const expression = `(() => {
      const containerId = window.SFConfig && SFConfig.CONTAINER_WINDOW_ID
        ? SFConfig.CONTAINER_WINDOW_ID.CURRENT_WINDOW
        : 0;
      const showByWindowMgr = window.SF && SF.windowMgr && typeof SF.windowMgr.show === "function";
      if (showByWindowMgr) {
        SF.windowMgr.show(containerId);
        return { ok: true, method: "SF.windowMgr.show", id: String(containerId) };
      }

      if (typeof window.ecShow === "function") {
        window.ecShow(containerId);
        return { ok: true, method: "ecShow", id: String(containerId) };
      }

      return { ok: false, reason: "No official show API is available", id: String(containerId) };
    })()`;
  ...
  async showOfficialWindowTarget(targetUrlPart, options = {}) {
    ...
    return this.showOfficialWindowPageTarget(target, options);
  }
...
export {
  ...
  parseNativeAlertDismissalOutput,
  parseNativeWindowConsolidationOutput,
  DEFAULT_PORT,
  DEFAULT_HOST,
  TOKEN_SALT,
  redactToken,
