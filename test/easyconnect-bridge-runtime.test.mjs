...
test("native UI AppleScript output parsers tolerate stable delimiter output", () => {
  assert.equal(parseNativeAlertDismissalOutput("2\n"), 2);
  assert.equal(parseNativeAlertDismissalOutput(""), 0);
  assert.deepEqual(parseNativeWindowConsolidationOutput("true##3##1##2\n"), {
    running: true,
    beforeCount: 3,
    afterCount: 1,
    closed: 2,
  });
  assert.deepEqual(parseNativeWindowConsolidationOutput("true\n2\n1\n1\n"), {
    running: true,
    beforeCount: 2,
    afterCount: 1,
    closed: 1,
