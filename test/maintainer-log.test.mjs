...
test("manual refresh audit events redact status snapshots", () => {
  const redacted = redactMaintainerEvent({
    event: "manual-refresh-status-completed",
    trigger: "button",
    status: {
      activeSession: {
        token: "secret-token",
      ...
      officialUi: {
        primaryTarget: { kind: "service" },
      },
    },
  });

  assert.equal(redacted.event, "manual-refresh-status-completed");
  assert.equal(redacted.status.activeSession.token, undefined);
  assert.deepEqual(redacted.status.loginStatus, { status: "1" });
  assert.deepEqual(redacted.status.serviceState, { base: "18", l3vpn: "18", tcp: "43" });
  assert.equal(redacted.status.officialUi.primaryTarget.kind, "service");
});
